                        ...
                        <div class="form-group">
                            ...
                            <select name="doctor_id" id="doctors" class="form-control select2 select2-hidden-accessible"
                                ...
                                @foreach ($doctors as $doctor)
                                    <option value="{{ $doctor->id }}" @if(old('doctor_id') == $doctor->id) selected @endif >{{ $doctor->first_name }}
                                        {{ $doctor->last_name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <select name="pateint_id" id="pateint" class="form-control select2 select2-hidden-accessible"
                                ...
                                @foreach ($patients as $patient)
                                    <option value="{{ $patient->id }}" @if(old('pateint_id') == $patient->id) selected @endif>{{ $patient->first_name }}
                            ...
                            <select name="status" id="status_patients"
                                ...
                                <option>{{ __('cms.select') }}</option>
                                <option value="1" @if(old('status') == 1) selected @endif>خطير</option>
                                <option value="2" @if(old('status') == 2) selected @endif>متوسط</option>
                                <option value="3" @if(old('status') == 3) selected @endif>عادي</option>
                            </select>
                            <label class="form-label">{{ __('cms.status_appointment') }}</label>
                            <select name="urgent" id="status_appointment"
                                class="form-control select2 select2-hidden-accessible" style="display: none;">
                                <option>{{ __('cms.select') }}</option>
                                <option value="1" @if(old('urgent') == 1) selected @endif>خطير</option>
                                <option value="0" @if(old('urgent') == 0) selected @endif>عادي</option>
                            </select>

<div class="form-group">
                                        <label class="form-label">{{__('cms.time')}} : </label>
                                        <input class="form-control" name="time" type="text" id="time" placeholder="{{__('cms.time')}} " value="{{old('time')}}">
                                    </div>

                                                                        <div class="form-group">
                                        <label class="form-label">{{__('cms.date')}} : </label>
                                        <input class="form-control" name="date" type="text" id="date" placeholder="{{__('cms.date')}} " value="{{old('date')}}">
                                    </div>
                        </div>
                    </div>
                        ...
                        <div class="wd-xl-100p ht-350">
                            <div class="ql-scrolling-demo bg-gray-100 ps" id="scrolling-container">
                                <textarea name="report" cols="30" rows="10" class="ql-container ql-bubble rtl"
                                    ...
                                    style="font-family: inherit; background: transparent; width: 100%; height: 100%; outline: 0;font-size: 19px; ">{{old('report')}}</textarea>
                            </div>
